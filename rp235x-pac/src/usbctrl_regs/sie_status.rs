...
#[doc = "Field `ENDPOINT_ERROR` reader - An endpoint has encounted an error. Read the ep_rx_error and ep_tx_error registers to find out which endpoint had an error."]
pub type ENDPOINT_ERROR_R = crate::BitReader;
#[doc = "Field `ENDPOINT_ERROR` writer - An endpoint has encounted an error. Read the ep_rx_error and ep_tx_error registers to find out which endpoint had an error."]
 ...
 * A setup packet is sent when no data in or data out transaction follows * An IN packet is received and the `LAST_BUFF` bit is set in the buffer control register * An IN packet is received with zero length * An OUT packet is sent and the `LAST_BUFF` bit is set"]
    ...
    #[doc = "Bit 23 - An endpoint has encounted an error. Read the ep_rx_error and ep_tx_error registers to find out which endpoint had an error."]
    #[inline(always)]
...
impl crate::Writable for SIE_STATUS_SPEC {
    ...
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0xff8e_1800;
}
