...
impl RegisterBlock {
    ...
    #[doc = "0x104 - Device only. Value of free-running PHY clock counter @48MHz when last SOF event occured."]
    #[inline(always)]
    ...
    #[doc = "0x114 - Watchdog that forces the device state machine to idle and raises an interrupt if the device stays in a state that isn't idle for the configured limit. The counter is reset on every state transition.  
...
pub mod ints;
#[doc = "SOF_TIMESTAMP_RAW (r) register accessor: Device only. Raw value of free-running PHY clock counter @48MHz. Used to calculate time between SOF events.  

You can [`read`](crate::Reg::read) this register and get [`sof_timestamp_raw::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "SOF_TIMESTAMP_LAST (r) register accessor: Device only. Value of free-running PHY clock counter @48MHz when last SOF event occured.  

You can [`read`](crate::Reg::read) this register and get [`sof_timestamp_last::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "SM_STATE (r) register accessor:   

You can [`read`](crate::Reg::read) this register and get [`sm_state::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
pub mod ep_rx_error;
#[doc = "DEV_SM_WATCHDOG (rw) register accessor: Watchdog that forces the device state machine to idle and raises an interrupt if the device stays in a state that isn't idle for the configured limit. The counter is reset on every state transition.  
 Set limit while enable is low and then set the enable.  
...
#[doc = "Watchdog that forces the device state machine to idle and raises an interrupt if the device stays in a state that isn't idle for the configured limit. The counter is reset on every state transition.  
 Set limit while enable is low and then set the enable."]
