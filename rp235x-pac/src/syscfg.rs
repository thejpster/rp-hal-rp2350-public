...
impl RegisterBlock {
    ...
    #[doc = "0x04 - For each bit, if 1, bypass the input synchronizer between that GPIO  
 ...
 latency. This register applies to GPIO 0...31."]
    ...
    #[doc = "0x08 - For each bit, if 1, bypass the input synchronizer between that GPIO  
 and the GPIO input register in the SIO. The input synchronizers should  
 generally be unbypassed, to avoid injecting metastabilities into processors.  
 If you're feeling brave, you can bypass to save two cycles of input  
 latency. This register applies to GPIO 32...47. USB GPIO 56..57 QSPI GPIO 58..63"]
    ...
    #[doc = "0x10 - Control PD pins to memories.  
 ...
 Use with caution"]
    ...
    #[doc = "0x14 - Spare registers, Just in case."]
    #[inline(always)]
    pub const fn spare(&self) -> &SPARE {
        &self.spare
    }
}
#[doc = "PROC_CONFIG (r) register accessor: Configuration for processors  
...
pub mod dbgforce;
#[doc = "MEMPOWERDOWN (rw) register accessor: Control PD pins to memories.  
 Set high to put memories to a low power state. In this state the memories will retain contents but not be accessible  
 Use with caution  
...
#[doc = "SPARE (rw) register accessor: Spare registers, Just in case.  
