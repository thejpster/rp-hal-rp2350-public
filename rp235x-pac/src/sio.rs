...
impl RegisterBlock {
 ...
 Value is 0 when read from processor core 0, and 1 when read from processor core 1."]
    ...
    #[doc = "0x04 - Input value for GPIO0...31.  
 ...
 In the NonSecure SIO, Secure-only GPIOs (as per ACCESSCTRL) appear as zero."]
    ...
    #[doc = "0x08 - Input value on GPIO32...47, QSPI IOs and USB pins  
 ...
 In the NonSecure SIO, Secure-only GPIOs (as per ACCESSCTRL) ignore writes, and their output status reads back as zero. This is also true for SET/CLR/XOR aliases of this register."]
    ...
    #[doc = "0x14 - Output value for GPIO32...47, QSPI IOs and USB pins.  
 ...
 Perform an atomic bit-set on GPIO_OUT, i.e. `GPIO_OUT |= wdata`"]
    ...
    #[doc = "0x1c - Output value set for GPIO32..47, QSPI IOs and USB pins.  
 ...
 Perform an atomic bit-clear on GPIO_OUT, i.e. `GPIO_OUT &amp;= ~wdata`"]
    ...
    #[doc = "0x24 - Output value clear for GPIO32..47, QSPI IOs and USB pins.  
 ...
 Perform an atomic bitwise XOR on GPIO_OUT, i.e. `GPIO_OUT ^= wdata`"]
    ...
    #[doc = "0x2c - Output value XOR for GPIO32..47, QSPI IOs and USB pins.  
 ...
 In the NonSecure SIO, Secure-only GPIOs (as per ACCESSCTRL) ignore writes, and their output status reads back as zero. This is also true for SET/CLR/XOR aliases of this register."]
    ...
    #[doc = "0x34 - Output enable value for GPIO32...47, QSPI IOs and USB pins.  
 ...
 Perform an atomic bit-set on GPIO_OE, i.e. `GPIO_OE |= wdata`"]
    ...
    #[doc = "0x3c - Output enable set for GPIO32...47, QSPI IOs and USB pins.  
 ...
 Perform an atomic bit-clear on GPIO_OE, i.e. `GPIO_OE &amp;= ~wdata`"]
    ...
    #[doc = "0x44 - Output enable clear for GPIO32...47, QSPI IOs and USB pins.  
 ...
 Perform an atomic bitwise XOR on GPIO_OE, i.e. `GPIO_OE ^= wdata`"]
    ...
    #[doc = "0x4c - Output enable XOR for GPIO32...47, QSPI IOs and USB pins.  
 ...
 The value returned on success is 0x1 &lt;&lt; lock number."]
    ...
    #[doc = "0x180 - Trigger a doorbell interrupt on the opposite core.  
 ...
 Read to get the status of the doorbells currently asserted on the opposite core. This is equivalent to that core reading its own DOORBELL_IN status."]
    ...
    #[doc = "0x184 - Clear doorbells which have been posted to the opposite core. This register is intended for debugging and initialisation purposes.  
 ...
 Reading returns the status of the doorbells currently asserted on the other core, i.e. is equivalent to that core reading its own DOORBELL_IN status."]
    ...
    #[doc = "0x18c - Check and acknowledge doorbells posted to this core. This core's doorbell interrupt is asserted when any bit in this register is 1.  
 ...
 Write 1 to each bit to clear that bit. The doorbell interrupt deasserts once all bits are cleared. Read to get status of doorbells currently asserted on this core."]
    ...
    #[doc = "0x190 - Detach certain core-local peripherals from Secure SIO, and attach them to NonSecure SIO, so that NonSecure software can use them. Attempting to access one of these peripherals from the Secure SIO when it is attached to the NonSecure SIO, or vice versa, will generate a bus error.  

 This register is per-core, and is only present on the Secure SIO.  

 Most SIO hardware is duplicated across the Secure and NonSecure SIO, so is not listed in this register."]
    #[inline(always)]
    pub const fn peri_nonsec(&self) -> &PERI_NONSEC {
        &self.peri_nonsec
    }
    #[doc = "0x1a0 - Control the assertion of the standard software interrupt (MIP.MSIP) on the RISC-V cores.  
 ...
 It is safe for both cores to write to this register on the same cycle. The set/clear effect is accumulated across both cores, and then applied. If a flag is both set and cleared on the same cycle, only the set takes effect."]
    ...
    #[doc = "0x1a4 - Control register for the RISC-V 64-bit Machine-mode timer. This timer is only present in the Secure SIO, so is only accessible to an Arm core in Secure mode or a RISC-V core in Machine mode.  
 ...
 Note whilst this timer follows the RISC-V privileged specification, it is equally usable by the Arm cores. The interrupts are routed to normal system-level interrupt lines as well as to the MIP.MTIP inputs on the RISC-V cores."]
    ...
    #[doc = "0x1b8 - Low half of RISC-V Machine-mode timer comparator. This register is core-local, i.e., each core gets a copy of this register, with the comparison result routed to its own interrupt line.  
 ...
 The timer interrupt is asserted whenever MTIME is greater than or equal to MTIMECMP. This comparison is unsigned, and performed on the full 64-bit values."]
    ...
    #[doc = "0x1bc - High half of RISC-V Machine-mode timer comparator. This register is core-local.  
 ...
 The timer interrupt is asserted whenever MTIME is greater than or equal to MTIMECMP. This comparison is unsigned, and performed on the full 64-bit values."]
    ...
    #[doc = "0x1c8 - Get the encoding of one pixel's worth of colour data, packed into a 32-bit value (3x10-bit symbols).  
 ...
 The PEEK alias does not shift the colour register when read, but still advances the running DC balance state of each encoder. This is useful for pixel doubling."]
    ...
    #[doc = "0x1cc - Get the encoding of one pixel's worth of colour data, packed into a 32-bit value. The packing is 5 chunks of 3 lanes times 2 bits (30 bits total). Each chunk contains two bits of a TMDS symbol per lane. This format is intended for shifting out with the HSTX peripheral on RP2350.  
 ...
 The POP alias shifts the colour register when read, as well as advancing the running DC balance state of each encoder."]
    ...
    #[doc = "0x1d0 - Get lane 0 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  
 ...
 The PEEK alias does not shift the colour register when read, but still advances the lane 0 DC balance state. This is useful if all 3 lanes' worth of encode are to be read at once, rather than processing the entire scanline for one lane before moving to the next lane."]
    ...
    #[doc = "0x1d4 - Get lane 0 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  
 ...
 The POP alias shifts the colour register when read, according to the values of PIX_SHIFT and PIX2_NOSHIFT."]
    ...
    #[doc = "0x1d8 - Get lane 1 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  
 ...
 The PEEK alias does not shift the colour register when read, but still advances the lane 1 DC balance state. This is useful if all 3 lanes' worth of encode are to be read at once, rather than processing the entire scanline for one lane before moving to the next lane."]
    ...
    #[doc = "0x1dc - Get lane 1 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  
 ...
 The POP alias shifts the colour register when read, according to the values of PIX_SHIFT and PIX2_NOSHIFT."]
    ...
    #[doc = "0x1e0 - Get lane 2 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  
 ...
 The PEEK alias does not shift the colour register when read, but still advances the lane 2 DC balance state. This is useful if all 3 lanes' worth of encode are to be read at once, rather than processing the entire scanline for one lane before moving to the next lane."]
    ...
    #[doc = "0x1e4 - Get lane 2 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  
...
#[doc = "Processor core identifier  
 Value is 0 when read from processor core 0, and 1 when read from processor core 1."]
pub mod cpuid;
#[doc = "GPIO_IN (r) register accessor: Input value for GPIO0...31.  
...
#[doc = "Input value for GPIO0...31.  
 ...
 In the NonSecure SIO, Secure-only GPIOs (as per ACCESSCTRL) appear as zero."]
pub mod gpio_in;
#[doc = "GPIO_HI_IN (r) register accessor: Input value on GPIO32...47, QSPI IOs and USB pins  
...
#[doc = "GPIO0...31 output value  
 ...
 In the NonSecure SIO, Secure-only GPIOs (as per ACCESSCTRL) ignore writes, and their output status reads back as zero. This is also true for SET/CLR/XOR aliases of this register."]
pub mod gpio_out;
#[doc = "GPIO_HI_OUT (rw) register accessor: Output value for GPIO32...47, QSPI IOs and USB pins.  
...
#[doc = "Output value for GPIO32...47, QSPI IOs and USB pins.  

 Write to set output level (1/0 -> high/low). Reading back gives the last value written, NOT the input value from the pins. If core 0 and core 1 both write to GPIO_HI_OUT simultaneously (or to a SET/CLR/XOR alias), the result is as though the write from core 0 took place first, and the write from core 1 was then applied to that intermediate result.  

 In the NonSecure SIO, Secure-only GPIOs (as per ACCESSCTRL) ignore writes, and their output status reads back as zero. This is also true for SET/CLR/XOR aliases of this register."]
pub mod gpio_hi_out;
#[doc = "GPIO_OUT_SET (w) register accessor: GPIO0...31 output value set  
 Perform an atomic bit-set on GPIO_OUT, i.e. `GPIO_OUT |= wdata`  
...
#[doc = "GPIO_HI_OUT_SET (w) register accessor: Output value set for GPIO32..47, QSPI IOs and USB pins.  
 Perform an atomic bit-set on GPIO_HI_OUT, i.e. `GPIO_HI_OUT |= wdata`  
...
#[doc = "Output value set for GPIO32..47, QSPI IOs and USB pins.  
 Perform an atomic bit-set on GPIO_HI_OUT, i.e. `GPIO_HI_OUT |= wdata`"]
pub mod gpio_hi_out_set;
#[doc = "GPIO_OUT_CLR (w) register accessor: GPIO0...31 output value clear  
 Perform an atomic bit-clear on GPIO_OUT, i.e. `GPIO_OUT &amp;= ~wdata`  
...
#[doc = "GPIO_HI_OUT_CLR (w) register accessor: Output value clear for GPIO32..47, QSPI IOs and USB pins.  
 Perform an atomic bit-clear on GPIO_HI_OUT, i.e. `GPIO_HI_OUT &amp;= ~wdata`  
...
#[doc = "Output value clear for GPIO32..47, QSPI IOs and USB pins.  
 Perform an atomic bit-clear on GPIO_HI_OUT, i.e. `GPIO_HI_OUT &amp;= ~wdata`"]
pub mod gpio_hi_out_clr;
#[doc = "GPIO_OUT_XOR (w) register accessor: GPIO0...31 output value XOR  
 Perform an atomic bitwise XOR on GPIO_OUT, i.e. `GPIO_OUT ^= wdata`  
...
#[doc = "GPIO_HI_OUT_XOR (w) register accessor: Output value XOR for GPIO32..47, QSPI IOs and USB pins.  
 Perform an atomic bitwise XOR on GPIO_HI_OUT, i.e. `GPIO_HI_OUT ^= wdata`  
...
#[doc = "GPIO0...31 output enable  
 ...
 In the NonSecure SIO, Secure-only GPIOs (as per ACCESSCTRL) ignore writes, and their output status reads back as zero. This is also true for SET/CLR/XOR aliases of this register."]
pub mod gpio_oe;
#[doc = "GPIO_HI_OE (rw) register accessor: Output enable value for GPIO32...47, QSPI IOs and USB pins.  
...
#[doc = "Output enable value for GPIO32...47, QSPI IOs and USB pins.  

 Write output enable (1/0 -> output/input). Reading back gives the last value written. If core 0 and core 1 both write to GPIO_HI_OE simultaneously (or to a SET/CLR/XOR alias), the result is as though the write from core 0 took place first, and the write from core 1 was then applied to that intermediate result.  

 In the NonSecure SIO, Secure-only GPIOs (as per ACCESSCTRL) ignore writes, and their output status reads back as zero. This is also true for SET/CLR/XOR aliases of this register."]
pub mod gpio_hi_oe;
#[doc = "GPIO_OE_SET (w) register accessor: GPIO0...31 output enable set  
 Perform an atomic bit-set on GPIO_OE, i.e. `GPIO_OE |= wdata`  
...
#[doc = "GPIO0...31 output enable set  
 Perform an atomic bit-set on GPIO_OE, i.e. `GPIO_OE |= wdata`"]
pub mod gpio_oe_set;
#[doc = "GPIO_HI_OE_SET (w) register accessor: Output enable set for GPIO32...47, QSPI IOs and USB pins.  
 Perform an atomic bit-set on GPIO_HI_OE, i.e. `GPIO_HI_OE |= wdata`  
...
#[doc = "Output enable set for GPIO32...47, QSPI IOs and USB pins.  
 Perform an atomic bit-set on GPIO_HI_OE, i.e. `GPIO_HI_OE |= wdata`"]
pub mod gpio_hi_oe_set;
#[doc = "GPIO_OE_CLR (w) register accessor: GPIO0...31 output enable clear  
 Perform an atomic bit-clear on GPIO_OE, i.e. `GPIO_OE &amp;= ~wdata`  
...
#[doc = "GPIO0...31 output enable clear  
 Perform an atomic bit-clear on GPIO_OE, i.e. `GPIO_OE &amp;= ~wdata`"]
pub mod gpio_oe_clr;
#[doc = "GPIO_HI_OE_CLR (w) register accessor: Output enable clear for GPIO32...47, QSPI IOs and USB pins.  
 Perform an atomic bit-clear on GPIO_HI_OE, i.e. `GPIO_HI_OE &amp;= ~wdata`  
...
#[doc = "GPIO_OE_XOR (w) register accessor: GPIO0...31 output enable XOR  
 Perform an atomic bitwise XOR on GPIO_OE, i.e. `GPIO_OE ^= wdata`  
...
#[doc = "GPIO0...31 output enable XOR  
 Perform an atomic bitwise XOR on GPIO_OE, i.e. `GPIO_OE ^= wdata`"]
pub mod gpio_oe_xor;
#[doc = "GPIO_HI_OE_XOR (w) register accessor: Output enable XOR for GPIO32...47, QSPI IOs and USB pins.  
 Perform an atomic bitwise XOR on GPIO_HI_OE, i.e. `GPIO_HI_OE ^= wdata`  
...
#[doc = "Reading from a spinlock address will:  
 ...
 The value returned on success is 0x1 &lt;&lt; lock number."]
pub mod spinlock;
#[doc = "DOORBELL_OUT_SET (rw) register accessor: Trigger a doorbell interrupt on the opposite core.  
...
#[doc = "DOORBELL_OUT_CLR (rw) register accessor: Clear doorbells which have been posted to the opposite core. This register is intended for debugging and initialisation purposes.  
...
#[doc = "Clear doorbells which have been posted to the opposite core. This register is intended for debugging and initialisation purposes.  
...
#[doc = "DOORBELL_IN_CLR (rw) register accessor: Check and acknowledge doorbells posted to this core. This core's doorbell interrupt is asserted when any bit in this register is 1.  
...
#[doc = "Check and acknowledge doorbells posted to this core. This core's doorbell interrupt is asserted when any bit in this register is 1.  

 Write 1 to each bit to clear that bit. The doorbell interrupt deasserts once all bits are cleared. Read to get status of doorbells currently asserted on this core."]
pub mod doorbell_in_clr;
#[doc = "PERI_NONSEC (rw) register accessor: Detach certain core-local peripherals from Secure SIO, and attach them to NonSecure SIO, so that NonSecure software can use them. Attempting to access one of these peripherals from the Secure SIO when it is attached to the NonSecure SIO, or vice versa, will generate a bus error.  
...
#[doc = "Detach certain core-local peripherals from Secure SIO, and attach them to NonSecure SIO, so that NonSecure software can use them. Attempting to access one of these peripherals from the Secure SIO when it is attached to the NonSecure SIO, or vice versa, will generate a bus error.  

 This register is per-core, and is only present on the Secure SIO.  

 Most SIO hardware is duplicated across the Secure and NonSecure SIO, so is not listed in this register."]
pub mod peri_nonsec;
#[doc = "RISCV_SOFTIRQ (rw) register accessor: Control the assertion of the standard software interrupt (MIP.MSIP) on the RISC-V cores.  
...
#[doc = "Control the assertion of the standard software interrupt (MIP.MSIP) on the RISC-V cores.  

 Unlike the RISC-V timer, this interrupt is not routed to a normal system-level interrupt line, so can not be used by the Arm cores.  

 It is safe for both cores to write to this register on the same cycle. The set/clear effect is accumulated across both cores, and then applied. If a flag is both set and cleared on the same cycle, only the set takes effect."]
pub mod riscv_softirq;
#[doc = "MTIME_CTRL (rw) register accessor: Control register for the RISC-V 64-bit Machine-mode timer. This timer is only present in the Secure SIO, so is only accessible to an Arm core in Secure mode or a RISC-V core in Machine mode.  
...
#[doc = "Control register for the RISC-V 64-bit Machine-mode timer. This timer is only present in the Secure SIO, so is only accessible to an Arm core in Secure mode or a RISC-V core in Machine mode.  
...
#[doc = "Read/write access to the high half of RISC-V Machine-mode timer. This register is shared between both cores. If both cores write on the same cycle, core 1 takes precedence."]
pub mod mtimeh;
#[doc = "MTIMECMP (rw) register accessor: Low half of RISC-V Machine-mode timer comparator. This register is core-local, i.e., each core gets a copy of this register, with the comparison result routed to its own interrupt line.  
...
#[doc = "Low half of RISC-V Machine-mode timer comparator. This register is core-local, i.e., each core gets a copy of this register, with the comparison result routed to its own interrupt line.  
 ...
 The timer interrupt is asserted whenever MTIME is greater than or equal to MTIMECMP. This comparison is unsigned, and performed on the full 64-bit values."]
pub mod mtimecmp;
#[doc = "MTIMECMPH (rw) register accessor: High half of RISC-V Machine-mode timer comparator. This register is core-local.  
...
pub mod tmds_ctrl;
#[doc = "TMDS_WDATA (w) register accessor: Write-only access to the TMDS colour data register.  

You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`tmds_wdata::W`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
pub mod tmds_wdata;
#[doc = "TMDS_PEEK_SINGLE (r) register accessor: Get the encoding of one pixel's worth of colour data, packed into a 32-bit value (3x10-bit symbols).  

 The PEEK alias does not shift the colour register when read, but still advances the running DC balance state of each encoder. This is useful for pixel doubling.  

You can [`read`](crate::Reg::read) this register and get [`tmds_peek_single::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "TMDS_POP_SINGLE (r) register accessor: Get the encoding of one pixel's worth of colour data, packed into a 32-bit value. The packing is 5 chunks of 3 lanes times 2 bits (30 bits total). Each chunk contains two bits of a TMDS symbol per lane. This format is intended for shifting out with the HSTX peripheral on RP2350.  

 The POP alias shifts the colour register when read, as well as advancing the running DC balance state of each encoder.  

You can [`read`](crate::Reg::read) this register and get [`tmds_pop_single::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "TMDS_PEEK_DOUBLE_L0 (r) register accessor: Get lane 0 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  

 The PEEK alias does not shift the colour register when read, but still advances the lane 0 DC balance state. This is useful if all 3 lanes' worth of encode are to be read at once, rather than processing the entire scanline for one lane before moving to the next lane.  

You can [`read`](crate::Reg::read) this register and get [`tmds_peek_double_l0::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "TMDS_POP_DOUBLE_L0 (r) register accessor: Get lane 0 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  

 The POP alias shifts the colour register when read, according to the values of PIX_SHIFT and PIX2_NOSHIFT.  

You can [`read`](crate::Reg::read) this register and get [`tmds_pop_double_l0::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "TMDS_PEEK_DOUBLE_L1 (r) register accessor: Get lane 1 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  

 The PEEK alias does not shift the colour register when read, but still advances the lane 1 DC balance state. This is useful if all 3 lanes' worth of encode are to be read at once, rather than processing the entire scanline for one lane before moving to the next lane.  

You can [`read`](crate::Reg::read) this register and get [`tmds_peek_double_l1::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "TMDS_POP_DOUBLE_L1 (r) register accessor: Get lane 1 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  

 The POP alias shifts the colour register when read, according to the values of PIX_SHIFT and PIX2_NOSHIFT.  

You can [`read`](crate::Reg::read) this register and get [`tmds_pop_double_l1::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "TMDS_PEEK_DOUBLE_L2 (r) register accessor: Get lane 2 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  

 The PEEK alias does not shift the colour register when read, but still advances the lane 2 DC balance state. This is useful if all 3 lanes' worth of encode are to be read at once, rather than processing the entire scanline for one lane before moving to the next lane.  

You can [`read`](crate::Reg::read) this register and get [`tmds_peek_double_l2::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
...
#[doc = "TMDS_POP_DOUBLE_L2 (r) register accessor: Get lane 2 of the encoding of two pixels' worth of colour data. Two 10-bit TMDS symbols are packed at the bottom of a 32-bit word.  

 The POP alias shifts the colour register when read, according to the values of PIX_SHIFT and PIX2_NOSHIFT.  

You can [`read`](crate::Reg::read) this register and get [`tmds_pop_double_l2::R`]. See [API](https://docs.rs/svd2rust/#read--modify--write-api).  
