...
#[doc = "Field `START` reader - Write 1 to prepare the SHA-256 core for a new checksum.  
...
#[doc = "Field `START` writer - Write 1 to prepare the SHA-256 core for a new checksum.  

 The SUMx registers are initialised to the proper values (fractional bits of square roots of first 8 primes) and internal counters are cleared. This immediately forces WDATA_RDY and SUM_VLD high.  

 START must be written before initiating a DMA transfer to the SHA-256 core, because the core will always request 16 transfers at a time (1 512-bit block). Additionally, the DMA channel should be configured for a multiple of 16 32-bit transfers."]
pub type START_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `WDATA_RDY` reader - If 1, the SHA-256 core is ready to accept more data through the WDATA register.  
...
#[doc = "Field `SUM_VLD` reader - If 1, the SHA-256 checksum presented in registers SUM0 through SUM7 is currently valid.  
...
#[doc = "Configure DREQ logic for the correct DMA data size. Must be configured before the DMA channel is triggered.  
...
#[doc = "Field `DMA_SIZE` reader - Configure DREQ logic for the correct DMA data size. Must be configured before the DMA channel is triggered.  
...
#[doc = "Field `DMA_SIZE` writer - Configure DREQ logic for the correct DMA data size. Must be configured before the DMA channel is triggered.  
...
#[doc = "Field `BSWAP` reader - Enable byte swapping of 32-bit values at the point they are committed to the SHA message scheduler.  
...
#[doc = "Field `BSWAP` writer - Enable byte swapping of 32-bit values at the point they are committed to the SHA message scheduler.  
...
impl R {
    #[doc = "Bit 0 - Write 1 to prepare the SHA-256 core for a new checksum.  
 ...
 START must be written before initiating a DMA transfer to the SHA-256 core, because the core will always request 16 transfers at a time (1 512-bit block). Additionally, the DMA channel should be configured for a multiple of 16 32-bit transfers."]
    ...
    #[doc = "Bit 1 - If 1, the SHA-256 core is ready to accept more data through the WDATA register.  
 ...
 After writing 16 words, this flag will go low for 57 cycles whilst the core completes its digest."]
    ...
    #[doc = "Bit 2 - If 1, the SHA-256 checksum presented in registers SUM0 through SUM7 is currently valid.  
 ...
 Goes low when WDATA is first written, then returns high once 16 words have been written and the digest of the current 512-bit block has subsequently completed."]
    ...
    #[doc = "Bits 8:9 - Configure DREQ logic for the correct DMA data size. Must be configured before the DMA channel is triggered.  
 ...
 The SHA-256 core's DREQ logic requests one entire block of data at once, since there is no FIFO, and data goes straight into the core's message schedule and digest hardware. Therefore, when transferring data with DMA, CSR_DMA_SIZE must be configured in advance so that the correct number of transfers can be requested per block."]
    ...
    #[doc = "Bit 12 - Enable byte swapping of 32-bit values at the point they are committed to the SHA message scheduler.  
