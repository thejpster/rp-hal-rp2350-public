#[doc = "Register `PAGE33_LOCK1` reader"]
pub type R = crate::R<PAGE33_LOCK1_SPEC>;
...
#[doc = "Lock state for NonSecure accesses to this page. Thermometer-coded, so lock state can be advanced permanently from any state to any less-permissive state by programming OTP. Software can also advance the lock state temporarily (until next OTP reset) using the SW_LOCKx registers.  

 Note that READ_WRITE and READ_ONLY are equivalent in hardware, as the SBPI programming interface is not accessible to NonSecure software. However, Secure software may check these bits to apply write permissions to a NonSecure OTP programming API.  

Value on reset: 0"]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
#[repr(u8)]
pub enum LOCK_NS_A {
    #[doc = "0: Page can be read by NonSecure software, and Secure software may permit NonSecure writes."]
    READ_WRITE = 0,
    #[doc = "1: Page can be read by NonSecure software"]
    READ_ONLY = 1,
    #[doc = "3: Page can not be accessed by NonSecure software."]
    INACCESSIBLE = 3,
}
...
#[doc = "Field `LOCK_NS` reader - Lock state for NonSecure accesses to this page. Thermometer-coded, so lock state can be advanced permanently from any state to any less-permissive state by programming OTP. Software can also advance the lock state temporarily (until next OTP reset) using the SW_LOCKx registers.  
...
impl LOCK_NS_R {
    ...
    #[doc = "Page can be read by NonSecure software, and Secure software may permit NonSecure writes."]
    #[inline(always)]
    pub fn is_read_write(&self) -> bool {
        *self == LOCK_NS_A::READ_WRITE
    }
    #[doc = "Page can be read by NonSecure software"]
    #[inline(always)]
    pub fn is_read_only(&self) -> bool {
        *self == LOCK_NS_A::READ_ONLY
    }
    #[doc = "Page can not be accessed by NonSecure software."]
    #[inline(always)]
    pub fn is_inaccessible(&self) -> bool {
...
impl R {
    ...
    #[doc = "Bits 2:3 - Lock state for NonSecure accesses to this page. Thermometer-coded, so lock state can be advanced permanently from any state to any less-permissive state by programming OTP. Software can also advance the lock state temporarily (until next OTP reset) using the SW_LOCKx registers.  
...
You can [`read`](crate::Reg::read) this register and get [`page33_lock1::R`](R). See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct PAGE33_LOCK1_SPEC;
impl crate::RegisterSpec for PAGE33_LOCK1_SPEC {
    type Ux = u32;
}
#[doc = "`read()` method returns [`page33_lock1::R`](R) reader structure"]
impl crate::Readable for PAGE33_LOCK1_SPEC {}
#[doc = "`reset()` method sets PAGE33_LOCK1 to value 0"]
impl crate::Resettable for PAGE33_LOCK1_SPEC {
